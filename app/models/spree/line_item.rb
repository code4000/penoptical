...
module Spree
  ...
  class LineItem < Spree::Base
    ...
    def amount
      @vision = order.vision_id >= 0 and order.vision_id <= 5
      @lens = order.lens_id >= 0 and order.lens_id <=4
      @package = order.package_id >= 0 and order.package_id <= 3

      if @vision || @lens || @package
        (price + order.vision.price + order.lens.price + order.package.price) * quantity
      else
        price * quantity
      end
    end
    ...
    def total
      if order.vision_id == nil
        amount + adjustment_total  
      elsif order.vision_id == 1
        (amount+1) + adjustment_total  
      end
      # amount + adjustment_total
    end
